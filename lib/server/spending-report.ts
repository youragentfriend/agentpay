...
export function buildSpendingReport(events: ActivityEvent[], options: Options = {}): SpendingReport {
  ...
  const transactions: SpendingReportTransaction[] = inRange.filter(event => event.spendState === "settled" || options.includePending).sort((a, b) => Date.parse(b.occurredAt) - Date.parse(a.occurredAt)).map(event => { const amount = cents(event.amountUsd); return { id: event.id, source: event.source, title: event.title, summary: event.summary, status: event.status, spendState: event.spendState as "settled" | "pending", amount: event.amount, asset: event.asset, amountUsd: amount === undefined ? "" : money(amount), occurredAt: event.occurredAt, reference: event.reference }; });
