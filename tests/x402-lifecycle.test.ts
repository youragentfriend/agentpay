...
import { resetSettingsStoreForTests,updateAgentPaySettings,updatePaymentExecutionControls } from "../lib/server/settings-store";
...
test("persists separate Prepare, Review, and Approve states and selected valuation",()=>withDatabase(async()=>{
 ...
 assert.equal(prepared.status,"prepared");const reviewed=reviewX402(prepared.id,1);assert.equal(reviewed.status,"reviewed");assert.equal(reviewed.amountUsd,"1.00");assert.equal(reviewed.selectedOption?.network,"eip155:8453");process.env.AGENTPAY_ENABLE_X402="true";updatePaymentExecutionControls({masterEnabled:true,rails:{"binance-pay":false,x402:true,"agentic-wallet":false},confirmEnable:true});const approved=approveX402(prepared.id);assert.equal(approved.status,"approved");assert.ok(approved.reviewedAt);assert.ok(approved.approvedAt);process.env.AGENTPAY_ENABLE_X402="false";await assert.rejects(()=>payX402(prepared.id),(error)=>error instanceof X402Error&&error.code==="PAYMENT_SERVER_DISABLED");
}));
