...
test("AI-unconfigured discovery is friendly and makes no provider request",async()=>{const provider=process.env.AGENTPAY_LLM_PROVIDER,key=process.env.AGENTPAY_LLM_API_KEY;delete process.env.AGENTPAY_LLM_PROVIDER;delete process.env.AGENTPAY_LLM_API_KEY;try{assert.equal(x402LlmStatus().configured,false);const result=await discoverWithAi("get weather",resources);assert.equal(result.configured,false);assert.deepEqual(result.candidateIds,[]);assert.match(result.message,/not configured/i);}finally{if(provider!==undefined)process.env.AGENTPAY_LLM_PROVIDER=provider;if(key!==undefined)process.env.AGENTPAY_LLM_API_KEY=key;}});
