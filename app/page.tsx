  ...
  return <main className="app-shell">
    ...
    <section className="content-area">
      ...
      <div className="page-content">
        ...
        {view === "x402" && <X402View/>}
        {view === "activity" && <ActivityView timeZone={settings.timeZone}/>}
...
function AssistantTransferCard({transfer,phase}:{transfer:PreparedTransfer;phase?:ChatTurn["transferPhase"]}){
  const submitted=Boolean(transfer.txHash)||phase==="submitted";
  const cancelled=phase==="cancelled"||transfer.errorCode==="USER_CANCELLED";
...
function WalletView({ onStatusChange }: { onStatusChange: (status: WalletConnectionStatus) => void }) {
  ...
  return <PageFrame eyebrow="Agentic Wallet" title="Your agentic wallet, under your control" description="Live balances, addresses, chains, and transaction history from Binance Agentic Wallet.">
...
function ConnectedWallet({ overview, onRefresh }: { overview: WalletOverview; onRefresh: () => Promise<void> }) {
  ...
  return <div className="wallet-dashboard">
    ...
    <section className="wallet-section"><div className="section-heading"><h2>Recent transactions</h2><span>Last {Math.min(5,overview.transactions.length)}</span></div>{overview.transactions.length ? <div className="data-list">{overview.transactions.slice(0,5).map((transaction) => <div className="data-row" key={transaction.txHash}><div><strong>{transaction.txType || "Transaction"}</strong><small className="mono-value"><TransactionExplorerLink chainId={transaction.binanceChainId} transactionHash={transaction.txHash} /></small></div><div className="amount"><strong className={`tx-status ${transaction.status}`}>{transaction.status}</strong><small>{transaction.txTime}</small></div></div>)}</div> : <p className="inline-empty">No recent transactions returned.</p>}</section></div>
  </div>;
}
...
function ActivityView({timeZone}:{timeZone:string}){return <PageFrame eyebrow="Activity" title="All activity" description="Filter approvals, transfers, Binance Pay payments, x402 purchases, successes, pending actions, and failures."><ActivityWorkflow timeZone={timeZone}/></PageFrame>}
