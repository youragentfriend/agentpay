  ...
  return <div className="overview-page">
    ...
    <section className="overview-primary-grid">
      ...
      <section className={`assistant-panel${turns.length ? " has-messages" : " empty"}`}>
        <div className="assistant-status"><div className="assistant-identity"><img className="assistant-badge" src="/brand/agentpay-mark.png" alt=""/><div><strong>AgentPay Assistant</strong><small><span className="status-dot active-dot"/> Ready for an instruction</small></div></div></div>
...
function RecentChats({conversations,onSelect}:{conversations:StoredConversation[];onSelect:(conversation:StoredConversation)=>void}){return <aside className="recent-chats-panel"><div className="panel-heading"><div><h2>Recent Chats</h2><p>Saved on this browser</p></div></div><div className="recent-chat-list">{conversations.length?conversations.map((conversation)=><button className="recent-chat-item" key={conversation.id} onClick={()=>onSelect(conversation)}><img className="recent-chat-icon" src="/brand/agentpay-mark.png" alt=""/><span><strong>{conversation.title}</strong><small>{new Date(conversation.savedAt).toLocaleString()}</small></span></button>):<div className="recent-chat-empty"><span>✦</span><strong>No saved conversations</strong><p>Click New conversation after chatting to save it here.</p></div>}</div></aside>}
