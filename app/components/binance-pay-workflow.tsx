...
export function BinancePayWorkflow({ initialFile = null, embedded = false, initialMode = "pay" }: { initialFile?: File | null; embedded?: boolean; initialMode?: "pay" | "receive" }) {
  ...
  const [sharingReceiveLink, setSharingReceiveLink] = useState(false);
  ...
  async function shareReceiveLink() {
    if (!receiveLink || sharingReceiveLink) return;
    setSharingReceiveLink(true);
    setShareError("");
    try {
      if (navigator.share) {
      ...
      if (shareError instanceof DOMException && shareError.name === "InvalidStateError") return;
      setShareError(shareError instanceof Error ? shareError.message : "Unable to share the receive link.");
    } finally { setSharingReceiveLink(false); }
  }
