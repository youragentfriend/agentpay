...
The Binance submenu also includes **Onchain Pay**, a distinct hosted Binance flow for sending Binance balances to external blockchain addresses. Asset and network choices are dynamic when approved partner credentials are configured, so the flow supports BNB and every other asset/network reported by Binance rather than a stablecoin-only list. Without partner access, the page uses a clearly labelled documented preview catalog. AgentPay validates the selected route, address, memo, amount, fee, and limits; persists a unique order; requires explicit approval; creates a Binance-hosted confirmation link only when `AGENTPAY_ENABLE_ONCHAIN_PAY=true`; verifies signed status webhooks; and projects lifecycle records into Activity and Reports. The pinned upstream skill is retained under `vendor/binance/onchain-pay`, but its shell helper is not executed because credentials would be process arguments and its formatting fallback can repeat a POST.

Onchain Pay requires a separately approved Binance Pay Onchain partner account and must not reuse Binance Portfolio or Binance Pay credentials. Configure `ONCHAIN_PAY_BASE_URL`, `ONCHAIN_PAY_CLIENT_ID`, `ONCHAIN_PAY_SIGN_ACCESS_TOKEN`, `ONCHAIN_PAY_PRIVATE_KEY_PATH`, `ONCHAIN_PAY_WEBHOOK_PUBLIC_KEY_PATH`, and approved redirect URLs through server-side protected configuration. Keep `AGENTPAY_ENABLE_ONCHAIN_PAY=false` until live discovery, webhook verification, order reconciliation, and a controlled QA transaction have passed.
