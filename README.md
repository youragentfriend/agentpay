...
AI routing uses the isolated `agentpay-bridge` OpenClaw agent with `deepseek/deepseek-v4-flash` and no fallbacks. AgentPay never receives the provider key: OpenClaw resolves the protected `DEEPSEEK_API_KEY` store SecretRef inside the official DeepSeek provider. DeepSeek native web search is not exposed by this provider version, so AI x402 discovery fails closed instead of substituting another provider or an unverified search path; deterministic catalog browsing and direct endpoint workflows remain available. See `docs/deepseek-bridge.md` for deployment and rollback.
